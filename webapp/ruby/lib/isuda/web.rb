...
#    use Rack::Lineprof
    ...
    set(:set_name) do |value|
      condition {
        ...
        if user_id
          name = RedisClient.call.get(user_id)

          unless name
            name = db.xquery(%| select name from user where id = ? |, user_id).first[:name]
            RedisClient.call.set(user_id, name)
          end
    ...
    helpers do
      ...
      def htmlify(content)
	keywords = RedisClient.call.lrange('keywords', 0, -1)
    ...
    get '/initialize' do
      ...
      keywords.each do |k|
	RedisClient.call.rpush('keywords', k[:keyword])
      end
    ...
    post '/keyword', set_name: true, authenticate: true do
      ...
      # Cache to redis
      RedisClient.call.rpush('keywords', keyword) unless all_keywords.include?(keyword) 

      bound = [@user_id, keyword, description, now, now, @user_id, keyword, description, now]
      db.xquery(%|
    ...
    def all_keywords
      RedisClient.call.lrange('keywords', 0, -1)
    end
  end
end
